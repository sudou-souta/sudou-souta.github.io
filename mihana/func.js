...
// ステータスを見る関数
function showStatus(HP,maya,money) {
  let str_hp = String(HP);
  let str_maya = String(maya);
  let str_money = String(money);
  showRetroAlert(`HP: ${str_hp}, MAYA: ${str_maya}, MONEY: ${str_money}`);
}
...
function work() {
  if (HP < 20) {
    ...
    save();
    showRetroAlert("体力は大事に使ってください。労働しました");
  }
}
...
// ホームレスが近くにいたら
function homeless_near_out(){
    if (near_homeless()){
        // flagがfalseなら表示、trueだったら表示しない                                                                                                                                                     
        if (flag == false){
            // 助けるか                                                                                                                                                                                    
            let res = window.confirm("ホームレスを助けますか?");
            // 助ける                                                                                                                                                                                      
            if (res == true){
                homeless_help();
                //ホームレスを削除                                                                                                                                                                         
